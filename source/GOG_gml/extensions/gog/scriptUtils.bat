...
:itemCopyTo srcPath destFolder
    ...
    for /f "delims=" %%a in ('dir /b /a:-d "%~1" 2^>nul') do (
        if "%%~a" == "%~nx1" (
            powershell -NoLogo -NoProfile -Command "New-Item -ItemType Directory -Force -Path (Split-Path -Parent $env:PS_DESTINATION); Remove-Item -Path $env:PS_DESTINATION; Copy-Item -Path $env:PS_SRCPATH -Destination $env:PS_DESTINATION -Force"
        )
    )
